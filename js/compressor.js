...
class Calldata {
  ...
  #compressPart(fromByte, toByte) {
    ...
    function addJustCopyCompress(resultCompress, amount) {
      if (amount !== 0) {
        ...
        resultCompress.description.push(createDesc(resultCompress.description, amount, "01"))
      }
    ...
    for (let i = fromByte; i <= toByte; ) {
      if (this.bytesInfo[i].zeroCompress.decompressedSize >= toByte - i + 1) {
        ...
        partCompress.description.push(new CompressDataDescription(i, toByte - i + 1, "00"))
        return partCompress
      }
        ...
        if (this.bytesInfo[i].storageCompress[j].decompressedSize <= toByte - i + 1) {
          ...
          if (isZeroCompress) {
            if (this.bytesInfo[i].storageCompress[j].range() > this.bytesInfo[i].zeroCompress.range()) {
              ...
              partCompress.description.push(createDesc(partCompress.description, zeroBytesAmount, "00"))
              i += zeroBytesAmount
            }
            ...
            partCompress.description.push(createDesc(partCompress.description, this.bytesInfo[i].copyCompress.decompressedSize, "01"))
  ...
  compress() {
    ...
    if (this.bytesInfo[0].zeroCompress.decompressedSize !== 0) {
      bestCompressForFirstNBytes[0] = new CompressData(new CompressDataPower(1, 1), [new CompressDataDescription(0, 1, "00")])
    } else {
      bestCompressForFirstNBytes[0] = new CompressData(new CompressDataPower(1, 2), [new CompressDataDescription(0, 1, "01")])
    }

    for (let i = 1; i < this.bytesInfo.length; i++) {
      bestCompressForFirstNBytes[i] = new CompressData(
        new CompressDataPower(bestCompressForFirstNBytes[i - 1].power.decompressedSize + 1, bestCompressForFirstNBytes[i - 1].power.compressedSize + 2),
        [...bestCompressForFirstNBytes[i - 1].description, new CompressDataDescription(i, 1, "01")]
      )

      for (let j = i; j >= Math.max(0, i - 63); j--) {
        const partCompress = this.#compressPart(j, i)

        const prefixCompress = new CompressData(new CompressDataPower(0, 0), [])
