...
class Calldata {
	constructor(data) {
		...
		if (
			BigInt(`0x${data}`).toString(16) !== (dataTrim0 === "" ? "0" : dataTrim0)
		) {
	...
	#compressPart(fromByte, toByte) {
		function createDesc(arrayDesc, amountBytes, method) {
			...
			if (arrayDesc.length !== 0) {
				...
				startByte =
					arrayDesc[prevDescIndex].startByte +
					arrayDesc[prevDescIndex].amountBytes;
			}
			return new CompressDataDescription(startByte, amountBytes, method);
		}
		...
		let partCompress = new CompressData(new CompressDataPower(0, 0), []);
		...
		for (let i = fromByte; i <= toByte; ) {
			if (this.bytesInfo[i].zeroCompress.decompressedSize >= toByte - i + 1) {
				...
				partCompress.description.push(
					new CompressDataDescription(i, toByte - i + 1, "00"),
				);
				return partCompress;
			}
			...
			if (this.bytesInfo[i].zeroCompress.decompressedSize !== 0) {
				if (
					this.bytesInfo[i].copyCompress.decompressedSize >= toByte - i + 1 ||
					this.bytesInfo[i].zeroCompress.range() >
						this.bytesInfo[i].copyCompress.range()
				) {
			...
			for (let j = 0; j < this.bytesInfo[i].storageCompress.length; j++) {
				if (
					...
					const isStorageRangeMoreThanCopyCompress =
						this.bytesInfo[i].storageCompress[j].range() >
						this.bytesInfo[i].copyCompress.range();

					if (
						!isZeroCompress &&
						!isStorageRangeMoreThanCopyCompress &&
						!isPaddingWithCopy
					) {
						continue;
					}
						...
						if (
							...
							partCompress.description.push(
								createDesc(
									...
									this.bytesInfo[i].storageCompress[j].decompressedSize,
									this.bytesInfo[i].storageCompress[j].compressedSize === 2
										? "10"
										: "11",
								),
							);
							...
							partCompress.description.push(
								createDesc(partCompress.description, zeroBytesAmount, "00"),
							);
							i += zeroBytesAmount;
						}
			...
			if (needJustCopyAmount) {
				const newJustCopyAmount = Math.min(
					this.bytesInfo[i].copyCompress.decompressedSize,
					toByte - i + 1,
				);
			...
			switch (instractions[i].method) {
				case "00":
					...
					result += scaleFraction(
						BigInt(instractions[i].amountBytes - 1).toString(16),
					);
					break;
				case "01": {
					// 01PXXXXX
					const copyBytes = this.getBytes(
						instractions[i].startByte,
						instractions[i].amountBytes,
					);
					...
					result += scaleFraction(
						BigInt(
							instractions[i].amountBytes -
								...
								1 +
								64 +
								(nonZeroByteIndex === 0 ? 0 : 32),
						).toString(16),
					);
					result += this.getBytes(
						instractions[i].startByte + nonZeroByteIndex,
						instractions[i].amountBytes - nonZeroByteIndex,
					);
					break;
				}
				case "10":
					...
					result += scaleFraction(
						BigInt(
							index +
								2 ** 15 +
								BB.indexOf(instractions[i].amountBytes) * 2 ** 12,
						).toString(16),
					);
					break;
				case "11":
					...
					result += scaleFraction(
						BigInt(
							index +
								3 * 2 ** 22 +
								BB.indexOf(instractions[i].amountBytes) * 2 ** 20,
						).toString(16),
					);
					break;
				default:
	...
	compress() {
		...
		for (let i = 1; i < this.bytesInfo.length; i++) {
			...
			for (let j = i; j >= Math.max(0, i - 63); j--) {
				...
				if (partCompress.description[0].startByte !== 0) {
					prefixCompress.power =
						bestCompressForFirstNBytes[
							partCompress.description[0].startByte - 1
						].power;
					prefixCompress.description =
						bestCompressForFirstNBytes[
							partCompress.description[0].startByte - 1
						].description;
				}
				...
				if (
					prefixCompress.power.range() + partCompress.power.range() >
					bestCompressForFirstNBytes[i].power.range()
				) {
					bestCompressForFirstNBytes[i] = new CompressData(
						new CompressDataPower(
							prefixCompress.power.decompressedSize +
								partCompress.power.decompressedSize,
							prefixCompress.power.compressedSize +
								partCompress.power.compressedSize,
						),
		...
		return {
			...
			compressedData: this.zip(
				bestCompressForFirstNBytes[this.bytesInfo.length - 1].description,
			),
	...
	checkZerosCase(n) {
		...
		currentByteIndex++;
		while (
			this.getByte(currentByteIndex) === "00" &&
			currentByteIndex < this.data.length / 2 &&
			currentByteIndex - n <= 63
		) {
			currentByteIndex++;
		}
	...
	checkCopyCaseWithZeros(n) {
		...
		while (
			this.getByte(currentByteIndex) === "00" &&
			currentByteIndex < this.data.length
		) {
		...
		return new CompressDataPower(
			decompressedBytesAmount,
			decompressedBytesAmount === 32
				? 1 + 32 - (currentByteIndex - n + 1)
				: 1 + decompressedBytesAmount,
		);
	}
