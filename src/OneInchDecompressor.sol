...
import { DecompressorDelegate } from "./DecompressorDelegate.sol";

/**
 * @title OneInchDecompressor
 ...
 */
abstract contract OneInchDecompressor is DecompressorDelegate {
    /**
     ...
     */
    function getData(uint256 begin, uint256 end)
        external
        view
        validDictAccess(begin)
        validDictAccess(end)
     ...
     */
    function _setDataArray(uint256 offset, bytes32[] calldata dataArray)
        internal
        validDictAccess(offset)
        validDictAccess(offset + dataArray.length)
    {
        unchecked {
