...
library RLPLib {
    ...
    function payloadLocation(bytes memory item)
        public
        pure
        returns (uint256 payloadMemPtr, uint256 payloadLen, uint256 itemMemPtr)
    {
        RLPReader.RLPItem memory rlpItem = item.toRlpItem();
    ...
    function rlpEncode(bytes memory item) public pure returns (bytes memory) {
        RLPReader.RLPItem memory rlpItem = item.toRlpItem();
        return rlpItem.toBytes();
    }
    ...
    function customDestructure(bytes memory item) public pure returns (address, bool, uint256) {
